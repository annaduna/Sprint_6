...
class MainPage(BasePage):
    ...
    def click_question(self,order_id):
        question_locator=QuestionsLocators.get_question_locator(order_id)
        self.click_on_element(question_locator)

    @allure.step('проверить текст ответа на вопрос')
    def check_answer_on_question(self, order_id, expected_answer):
        actual_text = self.get_text_on_element(QuestionsLocators.get_answer_locator(order_id))
        return actual_text == expected_answer

    @allure.step('скрол до вопроса')
    def scroll_to_question(self,order_id):
        question_locator= QuestionsLocators.get_question_locator(order_id)
        self.scroll_to_element(question_locator)

    @allure.step('подождать пока вопрос будет кликабелен')
    def wait_to_be_clic_question(self, order_id):
        question_locator = QuestionsLocators.get_question_locator(order_id)
        self.wait_for_element_to_be_clickable(question_locator)

    @allure.step('ожидание появления текста ответа на вопрос')
    def wait_answer_text(self,order_id):
        answer_locator = QuestionsLocators.get_answer_locator(order_id)
